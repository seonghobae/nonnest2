...
icci <- function(object1, object2, conf.level=.95, ll1=llcont, ll2=llcont) {
  ...
  if(!is.numeric(conf.level) || length(conf.level) != 1 || is.na(conf.level) || conf.level <= 0 || conf.level >= 1) stop("conf.level must be a single numeric value between 0 and 1", call. = FALSE)
