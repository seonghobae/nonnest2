...
vuongtest <- function(object1, object2, nested=FALSE, adj="none", ll1=llcont, ll2=llcont, score1=NULL, score2=NULL, vc1=vcov, vc2=vcov) {
  ...
  if(!is.logical(nested) || length(nested) != 1 || is.na(nested)) stop("nested must be a single logical value", call. = FALSE)
  if(!is.character(adj) || length(adj) != 1 || !(adj %in% c("none", "aic", "bic"))) stop("adj must be one of 'none', 'aic', or 'bic'", call. = FALSE)
