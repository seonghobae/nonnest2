...
llcont.hurdle <- function(x, ...) {
  ...
  zeroPoisson <- function(parms) {
    ...
    if(length(weights) == 1) weights <- rep_len(weights, length(Y1))
    if (any(Y1)) res[Y1] <- res[Y1] + weights[Y1] * log(1 - exp(loglik0[Y1]))
    res
  }
  ...
  countNegBin <- function(parms) {
    ...
    res <- Y1 * 0
    if(length(weights) == 1) weights <- rep_len(weights, length(Y1))
    if (any(Y1)) res[Y1] <- weights[Y1] * loglik1[Y1] - weights[Y1] * log(1 - exp(loglik0[Y1]))
    res
  }
