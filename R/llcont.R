...
llcont.glm <- function(x, ...){
    ...
    switch(fam,
           binomial = {
             ...
             m_cond <- m > 0
             if (any(m_cond)) {
               wt_vec <- if(length(wt) == 1) rep_len(wt, length(m)) else wt
               wt_res[m_cond] <- wt_vec[m_cond] / m[m_cond]
             }
             wt <- wt_res
...
llcont.hurdle <- function(x, ...) {
  ...
  zeroPoisson <- function(parms) {
    ...
    if (any(Y1)) {
      ...
      res[Y1] <- w_vec[Y1] * log(1 - exp(loglik0[Y1]))
    }
  ...
  countNegBin <- function(parms) {
    ...
    if (any(Y1)) {
      w_vec <- if(length(weights) == 1) rep_len(weights, length(Y1)) else weights
      res[Y1] <- w_vec[Y1] * loglik1[Y1] - w_vec[Y1] * log(1 - exp(loglik0[Y1]))
    }
    res
    }
