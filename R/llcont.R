...
llcont.hurdle <- function(x, ...) {
  ...
  zeroPoisson <- function(parms) {
    ...
    res <- loglik0 * 0
    if (any(Y1)) {
        if(length(weights) == 1) w_temp <- rep_len(weights, length(Y1)) else w_temp <- weights
        res[Y1] <- w_temp[Y1] * log(1 - exp(loglik0[Y1]))
    }
  ...
  countNegBin <- function(parms) {
    ...
    if (any(Y1)) {
        if(length(weights) == 1) w_temp <- rep_len(weights, length(Y1)) else w_temp <- weights
        res[Y1] <- w_temp[Y1] * loglik1[Y1] - w_temp[Y1] * log(1 - exp(loglik0[Y1]))
    }
    res
    }
