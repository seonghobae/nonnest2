    ...
    switch(fam,
           binomial = {
             if(is.matrix(y)) {
               ...
               n <- rowSums(y)
               ## Bolt: optimized ifelse overhead with vectorized subsetting
               y_res <- y[, 1] * 0
               idx <- as.logical(n != 0)
               if(any(idx)) y_res[idx] <- y[idx, 1]/n[idx]
               y <- y_res
             } else {
             ...
             ## Bolt: optimized ifelse overhead with vectorized subsetting
             wt_res <- wt * 0
             idx_m <- as.logical(m > 0)
             if(any(idx_m)){
                 m_vec <- if(length(m) == 1) rep_len(m, length(wt)) else m
                 wt_res[idx_m] <- wt[idx_m]/m_vec[idx_m]
             }
             wt <- wt_res
...
llcont.hurdle <- function(x, ...) {
  ...
  zeroPoisson <- function(parms) {
    ...
    idx <- as.logical(Y1)
    if(any(idx)) {
        w_vec <- if(length(weights) == 1) rep_len(weights, length(Y1)) else weights
        res[idx] <- w_vec[idx] * log(1 - exp(loglik0[idx]))
    }
  ...
  countNegBin <- function(parms) {
    ...
    if(any(idx)) {
        w_vec <- if(length(weights) == 1) rep_len(weights, length(Y1)) else weights
        res[idx] <- w_vec[idx] * loglik1[idx] - w_vec[idx] * log(1 - exp(loglik0[idx]))
    }
    res
    }
  ...
  for(g in 1:ngroups) {
    ...
    if(!samplestats@missing.flag) { # complete data
      if(x@Model@meanstructure) { # mean structure
        ...
        ## Bolt: replaced apply(..., 2, mean) with optimized colMeans() for performance
