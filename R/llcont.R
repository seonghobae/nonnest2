...
llcont.hurdle <- function(x, ...) {
  ...
  zeroPoisson <- function(parms) {
    ...
    loglik0 <- -mu
    ## Bolt: replaced ifelse() with vectorized subsetting for performance, safely handling scalar weights
    res <- Y1 * 0
    w_safe <- if(length(weights) == 1) rep_len(weights, length(Y1)) else weights
    if(any(Y1)) res[Y1] <- w_safe[Y1] * log(1 - exp(loglik0[Y1]))
  ...
  countNegBin <- function(parms) {
    ...
    res <- Y1 * 0
    w_safe <- if(length(weights) == 1) rep_len(weights, length(Y1)) else weights
    if(any(Y1)) res[Y1] <- w_safe[Y1] * loglik1[Y1] - w_safe[Y1] * log(1 - exp(loglik0[Y1]))
    res
    }
