    ...
    switch(fam,
           binomial = {
             if(is.matrix(y)) {
               ...
               y_res <- n * 0
               cond <- n != 0
               if (any(cond)) y_res[cond] <- y[cond, 1] / n[cond]
               y <- y_res
             } else {
             ...
             cond <- m > 0
             if(any(cond)) {
               w_wt <- if(length(wt) == 1) rep_len(wt, length(m)) else wt
               wt_res[cond] <- w_wt[cond] / m[cond]
             }
             wt <- wt_res
...
llcont.hurdle <- function(x, ...) {
  ...
  zeroPoisson <- function(parms) {
    ...
    res <- Y1 * 0
    if (any(Y1)) {
        w_Y1 <- if (length(weights) == 1) rep_len(weights, length(Y1)) else weights
        res[Y1] <- w_Y1[Y1] * log(1 - exp(loglik0[Y1]))
    }
  ...
  countNegBin <- function(parms) {
    ...
    if (any(Y1)) {
        w_Y1 <- if (length(weights) == 1) rep_len(weights, length(Y1)) else weights
        res[Y1] <- w_Y1[Y1] * loglik1[Y1] - w_Y1[Y1] * log(1 - exp(loglik0[Y1]))
    }
    res
    }
