...
llcont.hurdle <- function(x, ...) {
  ...
  zeroPoisson <- function(parms) {
    ...
    loglik0 <- -mu
    res <- Y1 * 0
    wt_adj <- if(length(weights) == 1) rep_len(weights, length(Y1)) else weights
    if(any(Y1)) res[Y1] <- wt_adj[Y1] * log(1 - exp(loglik0[Y1]))
  ...
  countNegBin <- function(parms) {
    ...
    res <- Y1 * 0
    wt_adj <- if(length(weights) == 1) rep_len(weights, length(Y1)) else weights
    if(any(Y1)) res[Y1] <- wt_adj[Y1] * loglik1[Y1] - wt_adj[Y1] * log(1 - exp(loglik0[Y1]))
    res
  }
